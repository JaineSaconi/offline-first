...
**Tipo:** Funções puras — Repositório de dados (tabela `outbox`)
...
- O limite de 20 itens por ciclo protege contra travamento em filas grandes.
...
- Define `doneAt = now`.
- Limpa `lastError`.
- Chamado dentro de `withTransactionAsync` pelo `runSync` junto com `markSynced`.
...
```ts
await updateRetry(id, 3, Date.now() + 8000, "Network timeout");
```

- Volta o status para `'PENDING'`.
- Incrementa `attempts`.
- Define `nextRetryAt` com o próximo tempo de retry (calculado com backoff pelo sync engine).
...
```ts
await markFailed(id, "404 Not Found");
```
